    ...
    <p className="mb-6 text-lg font-normal text-gray-500 lg:text-xl">
        ...
        <AutocompleteInput suggestions={suggestions} onSelect={
             (newJob) => {
                ...
                newState.secondaryJobTitle = newJob.text;
        ...
        <CardButton onClick={async () => {
            if (appContext && appContext.state.secondaryJobTitle != "" && appContext.state.secondaryJobTitle != null) {
                props.setView(() => FamilyView)
            
                const res = await fetch(API_BASE_URL + `api/getSalaryByJobAndLocation?us_state=${getStateFullName(appContext.state.secondaryState)}&occupation=${appContext.state.secondaryJobTitle}`)
                const salary = await res.json()
                const newState = appContext.state;
                newState.secondarySalary = salary.income;
                appContext.setState(newState)
        }
